...
if __name__ == '__main__':
    ...
    playtime = 6000
    repeat_time = 1
    learning_pram = [0.001, 0.005, 0.01, 0.05]
    ...
    fig = plt.figure()
    plt.plot(range(playtime), game1_Regret_ave, color='blue', label='beta={}'.format(learning_pram[0]))
    plt.plot(range(playtime), game2_Regret_ave, color='red', label='beta={}'.format(learning_pram[1]))
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig('comp_learning_param.png')
    fig.savefig('comp_learning_param.eps')
    #plt.show()